...
class Gram_Prac4:
    ...
    def getSiguiente(self, k, rec = False):
        if k in self.terminales:
            return "$"
        if k == rec:
            return False
        ...
        for p in self.Producciones:
            if k == p.izq:
                if p.der[0] in self.terminales:
                    if len(p.der) > 1:
                        answer.append(self.getPrimero(p.der[1]))
                    else:
                        answer.append([])
                elif p.der[0] in self.noterminales:
                    answer.append(self.getSiguiente(p.der[0], rec = k))
        answer = flatten(answer)
        a = {}
        for i in answer:
            a[i] = True
    ...
    def getSiguientes(self):
        answer = []
        for k, v in self.noterminales.items():
            answer.append([k, self.getSiguiente(k)])
        return answer
